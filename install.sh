...
run_checkbox_selection() {
    ...
    # Read input loop
    while true; do
        # Read single character using dd for better compatibility
        local key
        key=$(dd bs=1 count=1 2>/dev/null) || {
            # If dd fails, fall back to simple selection
            cursor_show
            restore_terminal
            run_simple_selection
            return $?
        ...
        case "$key" in
            ...
            q|Q)
                ...
                exit 0
                ;;
            # Escape key - start of escape sequence
            $'\x1b'|$'\033')
                # Read the rest of the escape sequence
                local seq
                seq=$(dd bs=2 count=1 2>/dev/null) || continue
                case "$seq" in
                    '[A') # Up arrow
                        move_up
                        redraw_menu
